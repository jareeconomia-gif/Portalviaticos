...
const DEFAULT_CONFIG = {
  ...
  requestTypes:['Viáticos anticipados','Reembolso'],
  paymentMethods:['Tarjeta corporativa','Transferencia electrónica','Reembolso posterior'],
  rates:{
  ...
  approvalRules:{skipDirectionBelowLimit:false,directionThreshold:0},
  questions:[
...
const DEFAULT_USERS = [
  ...
  },
  {
    ...
    jefeApproverId:'MASTER', direccionApproverId:'MASTER', approvalProfileId:'PROFILE_GENERAL'
  },
  {
    id:'DIRECCION', email:'direccion@empresa.com', password:'Direccion2026!', name:'Autorizador Dirección', role:'capturador',
    company:'Grupo Industrial Durandco', position:'Director Corporativo', baseCity:'CDMX', canApproveJefe:false, canApproveDireccion:false,
    jefeApproverId:'JEFE', direccionApproverId:'MASTER', approvalProfileId:'PROFILE_GENERAL'
  }
];
...
function sanitizeUserPayload(payload, row = null) {
  ...
  const user = {
    ...
    canApproveDireccion: isMaster,
    jefeApproverId: source.id === 'MASTER' ? '' : String(source.jefeApproverId || ''),
    direccionApproverId: 'MASTER'
  };
...
function normalizeRequest(input) {
  ...
  request.status = String(request.status || 'Pendiente Firma Jefe');
  request.jefeApproverId = String(request.jefeApproverId || '');
  request.direccionApproverId = 'MASTER';
...
function upsertRequest(request, userId) {
  db.prepare(`INSERT INTO requests(id,payload,requester_email,status,jefe_approver_id,direccion_approver_id,updated_at,updated_by)
    ...
    .run(request.id, JSON.stringify(request), request.requesterEmail, request.status, request.jefeApproverId, 'MASTER', nowIso(), userId);
}
function mergeApproverUpdate(oldRequest, incoming, user) {
  const next = clone(oldRequest);
  const fields = ['jefeComment','jefeSignedBy','jefeSignedAt','history'];
  for (const field of fields) if (Object.hasOwn(incoming, field)) next[field] = clone(incoming[field]);
  next.status = ['Pendiente Giovanni','Rechazada'].includes(String(incoming.status || '')) ? incoming.status : oldRequest.status;
  next.direccionApproverId = 'MASTER';
  next.requiresDirection = true;
  next.skipDirectionBelowLimit = false;
...
async function syncUsers(user, incomingUsers) {
  ...
  try {
    ...
    for (const row of existingRows) {
      if (row.id !== 'MASTER' && !seen.has(row.id)) db.prepare('UPDATE users SET active=0,updated_at=? WHERE id=?').run(stamp, row.id);
    }
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
  ...
  try {
    for (const incoming of normalized) {
      ...
      if (permission.mode === 'new-owner') {
        ...
        next.status = 'Pendiente Firma Jefe';
        next.direccionApproverId = 'MASTER';
      }
